...
export async function loadGLTF({ gpu, path }: { gpu: GPU; path: string }) {
    ...
    const gltf = (await await response.json()) as GLTFFormat;
    // gltf = await JSON.parse(JSON.stringify(gltf)) as GLTFFormat;

    const rootActor = new Actor({});

    // for debug
    // console.log('[loadGLTF]', gltf, gltf.scene, Object.values(gltf), Object.keys(gltf), Object.hasOwn(gltf, 'buffers'));

    // TODO: なぜかbuild時は変数に展開しないといけない...
    ...
    // @ts-ignore
    let scene: number;
    let scenes: GLTFScenes = [];
    let skins: GLTFSkines;

    for (const [key, value] of Object.entries(gltf)) {
        // console.log(key, value);
        switch (key) {
            case 'accessors':
                accessors = value as GLTFAccessor[];
                break;
            case 'animations':
                animations = value as GLTFAnimation[];
                break;
            case 'asset':
                asset = value as GLTFAsset;
                break;
            case 'bufferViews':
                bufferViews = value as GLTFBufferView[];
                break;
            case 'buffers':
                buffers = value as GLTFBuffer[];
                break;
            case 'meshes':
                meshes = value as GLTFMesh[];
                break;
            case 'nodes':
                nodes = value as GLTFNode[];
                break;
            case 'scene':
                scene = value as number;
                break;
            case 'scenes':
                scenes = value as GLTFScenes;
                break;
            case 'skins':
                skins = value as GLTFSkines;
                break;
        }
    }
    ...
    const binBufferDataList = await Promise.all(
        // gltf.buffers.map(async (buffer: GLTFBuffer) => {
        buffers.map(async (buffer: GLTFBuffer) => {
            // NOTE: buffer = { byteLength, uri }
            const binResponse = await fetch(buffer.uri);
            const binBufferData = await binResponse.arrayBuffer();
            return { byteLength: buffer.byteLength, binBufferData };
    ...
    const getBufferData = (accessor: GLTFAccessor) => {
        // const bufferView = gltf.bufferViews[accessor.bufferView];
        let bufferViewId: number = 0;
        for(const [key, value] of Object.entries(accessor)) {
            if(key === "bufferView") {
                bufferViewId = value as number;
            }
        }
        const bufferView = bufferViews[bufferViewId];
        // console.log("fugafuga", bufferView, bufferViews, accessor, accessor.bufferView)
    ...
    const createMesh = ({
        ...
        console.log('fuga', meshes);
        let primitives: GLTFMeshPrimitives[] = [];
        for (const [key, value] of Object.entries(mesh)) {
            if (key === 'primitives') {
                primitives = value as GLTFMeshPrimitives[];
            }
        }

        // mesh.primitives.forEach((primitive: GLTFMeshPrimitives) => {
        primitives.forEach((primitive: GLTFMeshPrimitives) => {
            const meshAccessors: GLTFMeshAccessor = {
                attributes: [],
                indices: null,
            };
            Object.keys(primitive.attributes).forEach((key) => {
                const attributeName = key as GLTFMeshAttributes;
                const accessorIndex = primitive.attributes[attributeName];
                console.log(primitives, attributeName, accessorIndex)
                if (accessorIndex === undefined || accessorIndex === null) {
                    console.error(`invalid accessor - attribute name: ${attributeName}`);
                    return;
                }
            ...
            if (primitive.indices) {
                // meshAccessors.indices = { accessor: gltf.accessors[primitive.indices] };
                meshAccessors.indices = { accessor: accessors[primitive.indices] };
            }
        ...
        if (skinIndex !== null) {
            console.log(`[loadGLTF.createMesh] mesh has skin: ${skinIndex}`);
            console.log(skins)
            console.log(skins[skinIndex].joints)
            let joints: number[] = [];
            ...
            for (const [key, value] of Object.entries(skin)) {
                if(key=== "joints") {
                    joints = value as number[];
                }
            }

            // NOTE: joints の 0番目が常に root bone のはず？
            // rootBone = createBone(skin.joints[0]);
            rootBone = createBone(joints[0]);
        }
    ...
    const findNode = (nodeIndex: number, parentActor: Actor): void => {
        // const targetNode: GLTFNode = gltf.nodes[nodeIndex];
        const targetNode: GLTFNode = nodes[nodeIndex];
        ...
        for (const [key, value] of Object.entries(targetNode)) {
            if (key === 'mesh') {
                mesh = value as number | undefined;
            }
            if (key === 'skin') {
                skin = value as number | undefined;
            }
        }

        console.log('target node', targetNode, targetNode.mesh, mesh);
        // mesh actor
        // if (hasMesh) {
        if (mesh !== null && mesh !== undefined) {
            ...
            const meshActor = createMesh({
                // nodeIndex,
                // meshIndex: targetNode.mesh!, // has mesh なのであるはず
                meshIndex: mesh, // has mesh なのであるはず
                // skinIndex: targetNode.hasOwnProperty('skin') ? targetNode.skin! : null, //
                // skinIndex: targetNode.skin !== null && targetNode.skin !== undefined ? targetNode.skin : null, //
                skinIndex: skin !== undefined ? skin : null, //
            });
            cacheNodes[nodeIndex] = meshActor;
    ...
    // gltf.scenes.forEach((scene: GLTFScene) => {
    console.log(scenes);
    scenes.forEach((scene: GLTFScene) => {
        let nodes: number[] = [];
        ...
        console.log('hogehoge', scene, scene.nodes, nodes);
        // scene.nodes.forEach((node) => {
        nodes.forEach((node) => {
            findNode(node, rootActor);
        });
    ...
    const createAnimationClips = () => {
        ...
        // return gltf.animations.map((animation: GLTFAnimation) => {
        return animations.map((animation: GLTFAnimation) => {
            let channels: GLTFAnimationChannel[] = [];
            let samplers: GLTFAnimationSampler[] = [];
            for (const [key, value] of Object.entries(animation)) {
                if (key === 'channels') {
                    channels = value as GLTFAnimationChannel[];
                }
                if (key === 'samplers') {
                    samplers = value as GLTFAnimationSampler[];
                }
            }
            const keyframes = channels.map((channel) => {
                let samplerId = 0;
                for (const [key, value] of Object.entries(channel)) {
                    if (key === 'sampler') {
                        samplerId = value as number;
                    }
                }
