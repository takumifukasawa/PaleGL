import { createMesh, getMeshMainMaterial, Mesh, MeshArgs } from '@/PaleGL/actors/mesh.ts';
import {
    ActorTypes,
    AttributeNames,
    AttributeUsageType,
...
export const updateSkinnedMesh: UpdateActorFunc = (actor: Actor, options: ActorUpdateArgs) => {
    ...
    if (skinnedMesh.boneLines && skinnedMesh.bonePoints) {
        ...
        // this.boneLines.geometry.updateAttribute(AttributeNames.Position, boneLinePositions.flat())
        // this.bonePoints.geometry.updateAttribute(AttributeNames.Position, boneLinePositions.flat())
        skinnedMesh.boneLines.geometry.updateAttribute(AttributeNames.Position, new Float32Array(boneLinePositions.flat()));
        skinnedMesh.bonePoints.geometry.updateAttribute(AttributeNames.Position, new Float32Array(boneLinePositions.flat()));
    }
    ...
    if (skinnedMesh.gpuSkinning) {
        ...
        skinnedMesh.materials.forEach((mat) => setMaterialUniformValue(mat, UniformNames.JointTexture, skinnedMesh.jointTexture));
        skinnedMesh.depthMaterials.forEach((depthMaterial) => {
