...
export function generateVolumetricLightParameters(
    params: VolumetricLightPassParametersArgs = {}
    ...
    return {
        ...
        blendRate: params.blendRate ?? 1,
        densityMultiplier: params.densityMultiplier ?? 1,
        rayJitterSize: params.rayJitterSize ?? createVector3(0.1, 0.1, 0.1),
        ratio: params.ratio ?? 0.5,
    };
}
...
export function createVolumetricLightPass(args: {
    gpu: Gpu;
    parameters?: VolumetricLightPassParametersArgs;
}): VolumetricLightPass {
    const { gpu } = args;
