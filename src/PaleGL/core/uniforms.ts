...
export function createUniforms(...dataArray: UniformsData[]) {
    ...
    // 新しい要素を追加
    const addValue = (name: string, type: UniformTypes, value: UniformValue) => {
        _data.push({
            name,
            type,
            value,
    ...
    // @param name
    // @param newValue
    const setValue = (name: string, newValue: UniformValue, log: boolean = false) => {
    ...
    const addUniformBlock = (blockIndex: number, uniformBufferObject: UniformBufferObject, data: UniformBufferObjectBlockData) => {
        // const blockIndex = uniformBufferObject.gpu.gl.getUniformBlockIndex(
        //     this.shader.glObject,
        // //     uniformBufferObject.blockName
        // );
