 ...
 * @param x
 */
export const gaussCoefficient = (sigma: number, x: number) => {
 ...
 * @param max
 */
export const clamp = (x: number, min: number, max: number) => Math.min(max, Math.max(x, min));
 ...
 * @param b
 */
export const randomRange = (a: number, b: number) => lerp(a, b, Math.random());
...
export const generateRandomValue = (seed: number, id: number): number => {
    // seedとidを組み合わせて一意の値にする
    const combinedSeed = seed ^ id;
    return mulberry32(combinedSeed);
};

export const randomOnUnitSphere = (seed: number, v: Vector3) => {
...
export const randomOnUnitPlane = (seed: number, scale: number = 1) => {
    const x = generateRandomValue(seed, 0) * 2 - 1;
    const z = generateRandomValue(seed, 1) * 2 - 1;
    return createVector3(x * scale, 0, z * scale);
};

export const randomOnUnitCircle = (id: number, scale: number, v: Vector3) => {
